...
def check_orientations(image, language, tessdata_dir_config, check_orientation):
    ...
    for angle in orientations:
        ...
        text, confidence = tesseract_ocr(rotated_image, language, tessdata_dir_config)
    ...
    if check_orientation == 2:
        for adjustment in range(1, MAX_ROTATION_STEPS + 1):
            ...
            adjusted_text, adjusted_confidence = tesseract_ocr(adjusted_image, language, tessdata_dir_config)
    ...
    with open(output_file, 'w', encoding='utf-8') as file_out:
        for index, filename in enumerate(tqdm(image_files, desc='Processing Images'), start=1):
            ...
            if check_orientation:
                text, final_angle, confidence = check_orientations(img, language, tessdata_dir_config, check_orientation)
            else:
                text, confidence = tesseract_ocr(img, language, tessdata_dir_config)
                final_angle = 0
...
def main():
    ...
    tessdata_dir = args.tessdata_path if args.tessdata_path else '/usr/share/tesseract-ocr/4.00/tessdata'
